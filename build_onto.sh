...
cat $ONTOLOGY_PATH/components/prefix.ttl | sed -e "s/|compiledate|/$(date --iso-8601=seconds)/g" |sed -e "s/+0000/Z/g"

for f in $(ls $ONTOLOGY_PATH/components/*.ttl|grep -v -w "prefix.ttl"); do
section "$(basename $f)"
cat $f
done
}
